...
def warpmethod_1(x, y):
    ...
    return 0.5 * np.sin(0.1 * x) + 0.5 * np.cos(0.1 * y)
class STLWarper:
    '''
    ...
    def preprocess(self):
        ...
        self.center_offset = [np.average(self.inputfile.mesh.bounds[:][0]), np.average(self.inputfile.mesh.bounds[:][1])]
    ...
    def slice_and_reverse(self, **kwargs):
        ...
        self.outputfile.save("data/stl/temp/warped.stl")
...
def read_and_warp_gcode(filename: str, warp_method: callable):
    '''
        ...
        if line.startswith("G1"):
            ...
            for part in parts:
                ...
                if part.startswith("X"):
                    ...
                    f = float(part[1:])
            new_z = z + z_mod * warp_method(x-110, y-110)
            if new_z < z_min:
                new_z = z_min
